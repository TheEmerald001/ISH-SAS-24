# Sascoin - a cryptocurrency implementation 
...
> Saschain uses websocket for p2p communication, but it was dropped to simplify the understanding of message exchange. It is relying only on REST communication.
...
It's the starting point to interact with the sascoin, and every node provides a swagger API to make this interaction easier. Available endpoints:
...
# Run two nodes
$ node bin/sascoin.js -p 3001 --name 1
$ node bin/sascoin.js -p 3002 --name 2 --peers http://localhost:3001
...
```sh
# Build the image
$ docker build . -t sascoin

# Run sascoin in a docker
$ ./dockerExec.sh

# Run sascoin in a docker using port 3002
$ ./dockerExec.sh -p 3002

# Run sascoin in a docker options
$ ./dockerExec.sh -h
...
$ node bin/sascoin.js -h
Usage: bin\sascoin.js [options]
...
```sh
# Cloning repository
$ git clone https://github.com/TheEmerald001/ISH-SAS-24.git
$ cd ISH-SAS-24
$ npm install
